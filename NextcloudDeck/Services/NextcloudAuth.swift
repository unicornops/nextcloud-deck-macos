...
final class NextcloudAuth {
    ...
    static func loginWithBrowser(serverURL: URL) async throws -> (serverURL: URL, loginName: String, appPassword: String) {
        let loginV2URL = serverURL.appendingPathComponent("index.php").appendingPathComponent("login").appendingPathComponent("v2")
        ...
        let pollURL = URL(string: initResponse.poll.endpoint) ?? serverURL.appendingPathComponent("login").appendingPathComponent("v2").appendingPathComponent("poll")
        ...
        while Date() < deadline {
            ...
            pollRequest.httpBody = "token=\(initResponse.poll.token.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? initResponse.poll.token)".data(using: .utf8)
    ...
    /// If the account already uses an app password, the server returns 403 — in that case the caller can use the provided password as the app password.
    static func getAppPassword(serverURL: URL, username: String, password: String) async throws -> String {
        let ocsURL = serverURL.appendingPathComponent("ocs").appendingPathComponent("v2.php").appendingPathComponent("core").appendingPathComponent("getapppassword")
...
enum AuthError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidEncoding: return "Invalid credential encoding"
        case .invalidResponse: return "Invalid response"
        case .serverError(let msg): return msg
        case .noAppPasswordInResponse: return "No app password in response"
        case .httpStatus(let code): return "HTTP \(code)"
        case .pollTimeout: return "Sign-in timed out. Complete sign-in in the browser and try again."
        }
    }
}
